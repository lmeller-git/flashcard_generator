...
def flashcards_from_large_file(file_, deck_name, mode, model):
            ...
            while _retry is True:
                try:
                    for _, _text_ in enumerate(text_):
                        try:
                            _answers += hub_chain.invoke({"input": _text_})["text"]  #hub_chain
                        except:
                            raise ValueError
                        try:
                            _questions += hub_chain_3.invoke({"input": _text_})["text"] #hub_chain_3
                        except:
                            _answers = ""
                            raise ValueError
                    try:
    ...
    if len(_images_) < len(flashcards):
        _images_ += [None for _ in range(len(flashcards)-len(images))]
