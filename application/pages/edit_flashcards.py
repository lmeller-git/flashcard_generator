...
with form:
    ...
    for i, card in enumerate(flashcards):
        ...
        excluded.append(st.toggle(f"Exclude this card", key = f"toggle_card_{i}", value = False))
...
def save_deck(cards = flashcards, deck_name = deck_name, excluded = excluded, save_data = save_data):
    ...
    def save_training_data(images_2, excluded):
        ...
        if not TRIAL_PATH.exists():
            ...
            _images_ = torch.empty(0)
            for i, image in enumerate(images):
                #IMAGE_SAVE_PATH_FOR_TRAINING = f"training_data/Images/image_{i + num_of_img}.png"
                IMAGE_SAVE_PATH_FOR_TRAINING = f"trial_dir/image_{i + num_of_img}.png"
                _image_tensor = transform(image)
                image_tensor_1, image_tensor_2, image_tensor_3 = _image_tensor[0] / 255, _image_tensor[1] / 255, _image_tensor[2] / 255
                #print(_image_tensor)
                #print(image_tensor_1)
                #print(len(_image_tensor))
                #image_tensor = SVD(_image_tensor / 255, 500)
                U_1, SIGMA_1, VT_1 = SVD(image_tensor_1, 32)
                U_2, SIGMA_2, VT_2 = SVD(image_tensor_2, 32)
                U_3, SIGMA_3, VT_3 = SVD(image_tensor_3, 32)
                _images_ = torch.([_images_, torch.stack([torch.stack([U_1, SIGMA_1, VT_1]),torch.stack([U_2, SIGMA_2, VT_2]),torch.stack([U_3, SIGMA_3, VT_3])])])
                #image_tensor = torch.stack([image_tensor_1, image_tensor_2, image_tensor_3])
                #image = transform_2(image_tensor)
                #image = image.save(fp = IMAGE_SAVE_PATH_FOR_TRAINING, format = "PNG")
        
        else:
            with open(TRIAL_PATH, "r") as f:
                _images_ = json.load(f)
                for i, image in enumerate(images):
                    #IMAGE_SAVE_PATH_FOR_TRAINING = f"training_data/Images/image_{i + num_of_img}.png"
                    IMAGE_SAVE_PATH_FOR_TRAINING = f"trial_dir/image_{i + num_of_img}.png"
                    _image_tensor = transform(image)
                    ...
                    U_2, SIGMA_2, VT_2 = SVD(image_tensor_2, 32)
                    U_3, SIGMA_3, VT_3 = SVD(image_tensor_3, 32)
                    _images_.update({f"image_{i}": [[U_1, SIGMA_1, VT_1],[U_2, SIGMA_2, VT_2],[U_3, SIGMA_3, VT_3]]})

            
        with open(f"trial_dir", "w") as f:
            torch.save(_images_, "images.pth")
        #print(_image_tensor)
        #print(image_tensor)
        assert False"""
        #### test ends here ####
        ...
        for i, image in enumerate(images_2):
            IMAGE_SAVE_PATH_FOR_TRAINING = f"training_data/Images/image_{i + num_of_img}.png"
            #image_tensor = SVD(transform(image), 200)
            #image = transform_2(image_tensor)
            image = image.save(fp = IMAGE_SAVE_PATH_FOR_TRAINING, format = "PNG")
...
with col1:
    ...
    reset_button = st.button(label = "delete decks")
with col2:
