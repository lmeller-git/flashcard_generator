...
def training_loop(epochs: int, model: torch.nn.Module, criterion: torch.nn.Module, optimizer: torch.nn.Module, train_data: torch.Tensor, device = "cpu", data_loader = DataLoader, batch_size: int = 1, shuffle = True):
...
if st.button("train ai"):
    ...
    training_loop(epochs, model.to(device), criterion, optimizer, train_data, test_data, device, batch_size=batch_size)
